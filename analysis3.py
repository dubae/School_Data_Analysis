...
class MainWindow(QMainWindow):
    def __init__(self, data):
        ...
        self.tab_widget.addTab(self.history_table, "2019~2022년 사고 데이터")
        self.tab_widget.addTab(self.chart_scroll_area, "원형 그래프")
    ...
    def show_accident_counts(self):
        ...
        for place, count in predicted_counts_2024.items():
            ...
            self.predicted_table.setItem(row, 0, self.create_table_item('2024'))
            self.predicted_table.setItem(row, 1, self.create_table_item(place))
            self.predicted_table.setItem(row, 2, self.create_table_item(f"{count:.2f} cases"))
            self.predicted_table.setItem(row, 3, self.create_table_item(f"{percentage:.2f}%"))
            row += 1
        ...
        for year, distribution in place_distribution.items():
            for place, percentage in distribution.items():
                ...
                self.history_table.setItem(row, 0, self.create_table_item(str(year)))
                self.history_table.setItem(row, 1, self.create_table_item(place))
                self.history_table.setItem(row, 2, self.create_table_item(f"{count} cases"))
                self.history_table.setItem(row, 3, self.create_table_item(f"{percentage:.2f}%"))
                row += 1
