...
class PlotCanvas(FigureCanvas):
    ...
    def plot(self, times, activity_distribution):
        self.axes.clear()
        for activity, data in activity_distribution.items():
            self.axes.plot(times, data, label=activity)
...
class AccidentActivityPage(QWidget):
    ...
    def show_accident_counts(self):
       ...
       # 그래프 데이터 준비
        plot_times = hours
        plot_data = {activity: [] for activity in activities}
        ...
        for hour in hours:
            ...
            for i, activity in enumerate(activities):
                ...
                plot_data[activity].append(percentage)
